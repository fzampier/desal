...
- Dual-model approach: Claude + GPT-5.4 (Codex)
...
- `scripts/orchestrate_extraction.py` — Dual-LLM extraction orchestrator (PDF → text/tables → Claude + GPT → verification)
- `scripts/compare_extractions.py` — Disagreement classifier (Levels 0-5)
- `scripts/llm_auditor.py` — LLM auditor layer for triage of Level 2-5 disagreements
- `pdfs/` — Included study PDFs (to be populated)
- `extracted_text/` — pdftotext outputs (generated)
- `extracted_tables/` — Table JSON files (generated)
- `data/` — Extraction outputs (extraction_log.json, disagreements.json, final_extractions.json)

### `analysis/` — Meta-analysis and TSA (Group 3)
- `R/prepare_data.R` — Convert final_extractions.json → analysis_ready.csv (with median/IQR conversion)
- `R/meta_analysis.R` — Random-effects MA (REML), forest/funnel plots, all subgroups + 8 sensitivity analyses
- `R/tsa.R` — Custom trial sequential analysis (O'Brien-Fleming, RIS, D² adjustment)
- `data/` — analysis_ready.csv (generated)
- `output/forest_plots/` — Forest plot PDFs (generated)
- `output/funnel_plots/` — Funnel plot PDFs (generated)
- `output/tsa_plots/` — TSA plot PDFs + PNGs (generated)

### `reporting/` — PRISMA and GRADE
- `prisma_flow.R` — PRISMA 2020 flow diagram annotated with dual-LLM metrics
- `grade_sof.R` — GRADE Summary of Findings table generator

### `clinical-data-extractor/` — Skill for verifying extracted data
- `SKILL.md` — Skill definition
- `scripts/` — Supporting scripts (anchor_extract.py, verify_numbers.py, verify_with_tables.py, benford_check.py, extract_tables.py)
